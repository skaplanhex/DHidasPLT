...
int PLTBinaryFileReader::ReadEventHits (std::ifstream& InFile, std::vector<PLTHit*>& Hits, unsigned long& Event, uint32_t& Time, uint32_t& BX)
{
  ...
  int wordcount = 0;
  bool bheader = true;
  while (bheader) { //while true at first

    // Read 64-bit word
    InFile.read((char *) &n2, sizeof n2); //read in the first 32 bits relative to the infile pointer, pointer now moved to end of these bits
    InFile.read((char *) &n1, sizeof n1); //read in the second 32 bits relative to the infile pointer, pointer now moved to the end of these bits

    if (InFile.eof()) { //if by reading those two sets of 32 bits we got to the end of the file, return -1 to stop looping over the file.
      return -1;
    }

    if ((n1 == 0x53333333) && (n2 == 0x53333333)) { //if both sets read 0x53333333, special handling of what's next
      //tdc buffer, special handling

      for (int ih = 0; ih < 100; ih++) { //for ih in range(100):

        InFile.read((char *) &n1, sizeof n1); //read in the next 32 bits after the second 0x53333333 and continuing each stage in the loop

        if ((n1 & 0xf0000000) == 0xa0000000) { //if these 32 bits pass this condition
          InFile.read((char *) &n2, sizeof n2); //seems like we found what we're looking for. Read in the next 32 bits and break the loop
          //doesn't seem like we're doing anything with this though, so whatever
          break;
        }
        if (InFile.eof()) { //if in the loop we get to the end of the file, stop reading the file
          return -1;
        }
      }

    } else if ( ((n1 & 0xff000000) == 0x50000000 && ((n2 & 0xfff00) >> 8) == 5 && (n2 & 0xff) == 0 ) || ( ((n2 & 0xff000000) == 0x50000000) && ((n1 & 0xfff00) >> 8) == 5 ) && (n1 & 0xff) == 0 ){
      // if either n1 or n2 have the correct signature from the FED
